<script>
    ...
    $(document).ready(function() {
        table = $("#DataProduk").DataTable({
            ...
            "serverSide": true,
            "ajax": {
                url: "<?= base_url("admin/barang/getdata") ?>",
                type: "post"
            },
            "columns": [{
                    "data": null,
        ...
        $("#DataProduk tbody").on("click", "tr", function() {
            ...
            let modalbodycontent = `<div class="row">
                                    ...
                                    <div class="col-lg-12"><button class="btn btn-outline-info" onclick="Diskon('${data.kode}')" style="width: 100%;"><i class="bi bi-percent"></i> Tambah Diskon</button></div>
                                </div>`;
    ...
    function ExportExcel() {
        ...
        $("#excel").submit(function(){
            $.ajax({
                url: "<?= base_url("admin/barang/export") ?>",
                data: {
                    tanggalawal: $("#awal").val(),
                    tanggalakhir: $("#akhir").val()
                },
                type: "post",
    ...
    function Diskon(kode) {
        ...
        swal.fire({
            title: "Isi Diskon Yang Ingin Di Berikan",
            text: "Jika Ingin Mengembalikan Menjadi 0% Cukup Ketik 0",
            input: "number",
            ...
            preConfirm: (diskon) => {
                return $.ajax({
                    type: "post",
                    url: '<?= base_url("admin/barang/diskon") ?>',
                    data: {
                        kode: kode,
                        diskon: diskon,
    ...
    function Stok(kode) {
        ...
        swal.fire({
            ...
            preConfirm: (stok) => {
                return $.ajax({
                    type: "post",
                    url: '<?= base_url("admin/barang/stok") ?>',
                    data: {
                        kode: kode,
                        stok: stok,
                        namabarang: data.namabarang
